...
> ekwlr **FFT + 특징 추출까지** 구현되어 있으며, 머신러닝/분류/예측 코드는 포함하지 않습니다.
...
FFT 파이프라인과 별도로 DWT 특징 및 CWT 시각화를 생성할 수 있습니다.
기본 설정은 `db4`, decomposition level 5, 파일당 4096 samples입니다.

```bash
cd src

# 정상/불량 대표 샘플 발표 이미지
python presentation_wavelet_pipeline.py

# 일부 파일로 특징 추출 확인
python wavelet_pipeline.py --limit 100 --overwrite

# 라벨별 동일 개수의 빠른 비교용 데이터 생성
python wavelet_pipeline.py --per-class-limit 200 --output ../outputs/features/features_wavelet_prototype_labeled.csv --overwrite

# 전체 원시 데이터에서 Wavelet 특징 생성
python wavelet_pipeline.py --overwrite

# Random Forest 학습 및 평가
python train_wavelet_rf.py --task multiclass
python train_wavelet_rf.py --task binary

# 설비 단위 일반화 성능 확인
python train_wavelet_rf.py --task multiclass --group-split
```

주요 결과:

- 특징 CSV: `outputs/features/features_wavelet_labeled.csv`
- 발표 이미지: `outputs/presentation/wavelet/`
- 모델: `models/wavelet/`
- 평가 결과: `outputs/ml/wavelet/`

`wavelet_pipeline.py`는 기존 출력 파일에 중복 행이 생기는 것을 막기 위해,
출력 CSV가 이미 있으면 중단합니다. 새로 생성할 때 `--overwrite`를 사용하세요.
`--per-class-limit` 결과는 기능 확인 및 빠른 실험용이며 최종 성능 보고에는 전체
데이터 또는 설비 단위 group split 결과를 사용해야 합니다.
